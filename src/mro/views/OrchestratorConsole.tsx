...
import { agentsById, specialistAgents, type AgentId } from "@/mro/data/agents";
...
import { AgentFlowDiagram } from "@/mro/components/agents/AgentFlowDiagram";
import { CardHeader } from "@/mro/components/agents/ConsoleKit";
import { FreightDashboard } from "@/mro/components/blocks/FreightDashboard";

/* ──────────────────────────────────────────────────────────────────────────
 * Freight Settlement Orchestrator console.
 *
 * The coordinator — no autonomy dial, no ceremony. It keeps shared context
 * across the five agents, posts in-tolerance shipments touchless and routes
 * every exception a person must own. Data surface: process metrics · the live
 * agent-flow map · each agent's current output state · the cross-agent
 * exception queue · chat.
 * ────────────────────────────────────────────────────────────────────────── */

type Metric = { label: string; value: string; sub: string };
const metrics: Metric[] = [
  { label: "Touchless rate", value: "82%", sub: "target 85%" },
  { label: "Settlement cycle time", value: "4.2h", sub: "was 9 days" },
  { label: "On-time settlement", value: "88%", sub: "up from 71%" },
  { label: "Overcharge recovered", value: "6.8%", sub: "vs carrier invoices" },
...
const exceptions: Exc[] = [
  { id: "po", title: "TNDR-77310 · over-threshold settlement", detail: "Rate matches contract · waiting on your sign-off", tag: "Decision" },
  { id: "sourcing", title: "CHI→RIV · off-contract surcharge spike", detail: "Above the managed-lane threshold → freight manager", tag: "Sign-off" },
  { id: "vendor", title: "Summit Carriers BOL mismatch · conf 0.88", detail: "Below the auto-post bar → load reviewer", tag: "Review" },
  { id: "invoice", title: "INV-SC-0991 · $310 overcharge variance", detail: "Root cause found · carrier note drafted", tag: "Proposed" },
];
...
const chatScript: ChatTurn[] = [
  {
    reply: [
      {
        kind: "agent",
        tone: "mint",
        text: "I'm the Approval & Exception Router — I coordinate the five agents, post in-tolerance shipments touchless and route the exceptions that need you. Ask me where things stand.",
        children: (
          <div className="text-[12.5px] text-ink leading-[19px]">
            <div className="text-mute mb-1">For example —</div>
            <ul className="space-y-0.5">
              <li>· Where's the CHI→RIV settlement now</li>
    ...
    chips: ["Where's the CHI→RIV settlement?", "What needs me today?", "How touchless are we?"],
  },
  {
    reply: [
      {
        kind: "agent",
        tone: "mint",
        text: "The CHI→RIV OCC load is mid-pipeline — Load capture matched the BOL, Rate & surcharge cleared the linehaul and the Carrier tender agent is holding TNDR-77310 for your approval. I'm carrying the shared context across all three.",
      },
    ],
    ...
    reply: [
      {
        kind: "agent",
        tone: "fog",
        text: "Three things: the over-threshold settlement sign-off, an off-contract surcharge spike on CHI→RIV and a Summit Carriers BOL mismatch the Load capture agent couldn't auto-post. Everything else is settling touchless.",
      },
    ],
    ...
    reply: [
      {
        kind: "agent",
        tone: "mint",
        text: "82% of shipments settled end-to-end with no human touch this month, median cycle time 4.2 hours — down from 9 days. The 18% that stop are the exceptions I route to you here, where we've recovered the overcharges and avoided demurrage.",
      },
    ],
  },
];
          ...
          <div className="flex items-center justify-between gap-4 rounded-md bg-white border border-divider px-5 py-4">
            <div className="min-w-0">
              ...
              <p className="text-[12px] text-mute leading-snug mt-0.5">
                The orchestrator coordinates all five agents and routes the one decision to you.
              </p>
            </div>
            <PillButton variant="deep" size="sm" arrow onClick={() => go({ kind: "workspace", flow: "belt" })}>
              <span className="inline-flex items-center gap-1">
                Open the settlement run <ChevronRight size={14} />
              </span>
            </PillButton>
          </div>
        </div>
