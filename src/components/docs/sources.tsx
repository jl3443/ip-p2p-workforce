...
export function EmailDoc({
  ...
  return (
    <DocShell>
      <div
        className={cn(
          "px-4 py-3 text-white flex items-start justify-between gap-4",
          tone === "outbound" ? "bg-[#0a6ed1]" : "bg-[#354a5f]",
        )}
      >
        <div className="leading-tight min-w-0">
          <div className="text-[14px] font-bold truncate">{subject}</div>
          <div className="text-[11px] opacity-85 mt-1">{tone === "outbound" ? "Sent message" : "Inbox"} · Outlook</div>
        </div>
        <div className="text-right text-[11px] opacity-85 leading-snug shrink-0">{sent}</div>
      </div>
      <div className="px-4 py-3 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1.5 border-b border-divider">
        <span className="text-[11px] text-mute">From</span>
        <span className="text-[12.5px] text-ink">
          <span className="font-medium">{from}</span> <span className="text-mute">· {fromAddr}</span>
        </span>
        <span className="text-[11px] text-mute">To</span>
        <span className="text-[12.5px] text-ink">{to}</span>
      </div>
